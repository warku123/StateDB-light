package logic

import (
	"context"

	"statedbl/service/sdb/api/internal/svc"
	"statedbl/service/sdb/api/internal/types"
	"statedbl/service/sdb/rpc/types/sdb"

	"github.com/zeromicro/go-zero/core/logx"
)

type SubRefundLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewSubRefundLogic(ctx context.Context, svcCtx *svc.ServiceContext) *SubRefundLogic {
	return &SubRefundLogic{
		Logger: logx.WithContext(ctx),
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

func (l *SubRefundLogic) SubRefund(req *types.SubRefundRequest) (resp *types.SubRefundResponse, err error) {
	// todo: add your logic here and delete this line
	_, err = l.svcCtx.SdbRpc.SubRefund(l.ctx, &sdb.SubRefundRequest{
		Amount: req.Amount,
	})

	if err != nil {
		return nil, err
	}

	return &types.SubRefundResponse{}, nil
}
